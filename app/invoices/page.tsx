...
type SortKey = "date" | "client_name" | "bank_company_name" | "total_amount";
...
export default function OverallInvoices({ params }: { params: { clientName: string } }) {
  ...
  const filteredData = [...invoiceData]
    .filter((inv) =>
      inv.client_name.toLowerCase().includes(debouncedSearch.toLowerCase()) ||
      (inv.bank_company_name?.toLowerCase() ?? "").includes(debouncedSearch.toLowerCase())
    )
    .sort((a, b) => {
      if (sortKey === "date") {
  ...
  useEffect(() => {
    document.title = "Overall Invoice Management";
  }, []);
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel("invoice-channel-overall")
  ...
  return (
    <main className="flex flex-col h-full w-full dark:bg-gray-950 p-4 gap-3 overflow-hidden">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="font-bold text-xl text-gray-900 dark:text-white">All Invoices</h1>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Button
            ...
            variant="outline"
            disabled={totalAmount === 0}
            onClick={() => exportInvoicesToPdf(invoiceData, totalAmount, "Overall", selectedDate.month, selectedDate.year, true)}
          >
            Export PDF
          </Button>
        </div>
          ...
          <Table>
            <TableHeader className="bg-gray-900 dark:bg-gray-800 sticky top-0 z-10">
              <TableRow className="border-0 hover:bg-transparent">
                {[
                  ...
                  { h: "Bank / Finance", w: "min-w-[130px]", key: "bank_company_name" as SortKey },
              ...
              {filteredData.length === 0 ? (
                    ...
                    <div className="text-gray-400 dark:text-gray-500">
                      ...
                      <p className="text-sm">
                        {searchQuery
                          ...
                          : `No invoices for ${MONTHS[selectedDate.month]} ${selectedDate.year}`}
                      </p>
                    </div>
                  </TableCell>
                </TableRow>
                ...
                filteredData.map((data, index) => (
                  ...
                  >
                    ...
                    <TableCell className="text-sm text-gray-600 dark:text-gray-300 py-3">{data.bank_company_name ?? "—"}</TableCell>
            ...
            {totalAmount > 0 && (
              <TableFooter className="bg-gray-50 dark:bg-gray-900 border-t-2 border-gray-200 dark:border-gray-700">
                <TableRow>
                  <TableCell colSpan={8} className="font-semibold text-gray-700 dark:text-gray-300 py-3">
